...
- (UILabel *)unitLabel {
    if (_unitLabel == nil) {
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(UUYLabelwidth, 0, 100, 40)];
...
- (void)setXLabels:(NSArray *)xLabels {
    _xLabels     = xLabels;
    _xLabelWidth = (self.myScrollView.frame.size.width - UUYLabelwidth * 0.5)/xLabels.count;

    for (int i = 0; i < xLabels.count; i++) {
        if (i%self.intervalValue == 0) {
            NSString *labelText = xLabels[i];
            YzcLabel *label     = [[YzcLabel alloc] initWithFrame:CGRectMake(i * self.xLabelWidth+UUYLabelwidth*0.5, self.frame.size.height - UULabelHeight, self.xLabelWidth+10, UULabelHeight)];
    ...
    if (self.targetValue) {  //如果设置了目标值就绘制目标虚线
        CGFloat chartCavanHeight = self.frame.size.height - UULabelHeight * 3 + 8;
...
- (void)setYLabels:(NSArray *)yLabels {
    ...
    if (self.targetValue) {
        CGFloat  chartCavanHeight = self.frame.size.height - UULabelHeight * 3 + 8;
        float    percent          = ((float)self.targetValue-_yValueMin) / ((float)_yValueMax-_yValueMin);
        YzcLabel *label           = [[YzcLabel alloc] initWithFrame:CGRectMake(0, (1 - percent) * chartCavanHeight+22, UUYLabelwidth+20, UULabelHeight)];
    ...
    if (self.style == BarChartStyleRateRange) {
        ...
        CGFloat chartCavanHeight = self.frame.size.height - UULabelHeight * LINE_COUNT + 8;
        CGFloat levelHeight      = level;//chartCavanHeight / LINE_COUNT ;

        for (int i = 1; i < LINE_COUNT+1; i++) {
            YzcLabel *label = [[YzcLabel alloc] initWithFrame:CGRectMake(0, chartCavanHeight - i * levelHeight + 13, UUYLabelwidth+20, UULabelHeight)];
        ...
        for (int i = 0; i < LINE_COUNT+1; i++) {
            ...
            if (i < LINE_COUNT) {
                [self drawDashLine:self.myScrollView
                             point:CGPointMake(25, UULabelHeight+i*levelHeight)
                        lineLength:2
    ...
    [self drawSolideLineWithMoveToPoint:CGPointMake(25, self.frame.size.height-UULabelHeight-10)
                            lineToPoint:CGPointMake(self.frame.size.width, self.frame.size.height-UULabelHeight-10)
...
- (void)strokeChart {
    ...
    _xLabelWidth = (self.myScrollView.frame.size.width - UUYLabelwidth * 0.5)/self.yLabels.count;

    for (int i = 0; i < self.yLabels.count; i++) {
        YzcBar *bar = [[YzcBar alloc] initWithFrame:CGRectMake(i * _xLabelWidth+UUYLabelwidth*0.5, UULabelHeight, 8, chartCavanHeight)];
